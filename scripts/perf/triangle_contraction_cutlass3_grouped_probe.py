#!/usr/bin/env python3
"""CUTLASS 3 SM90 grouped screen for the v2 triangle contraction core.

The old ``cutlass::gemm::device::GemmGrouped`` probe used an SM80-era wrapper
and lost to dense PyTorch.  This probe instead drives the CUTLASS 3 Hopper path:

* ``GroupProblemShape<Shape<int,int,int>>`` supplies one exact ``M,N,K`` per
  ``(feature, sequence)`` matrix.
* ``RowMajor*`` / ``ColumnMajor*`` layout tags make CUTLASS use pointer-backed
  per-group strides.
* ``KernelPtrArrayTmaWarpSpecializedCooperative`` selects the SM90 persistent
  TMA grouped scheduler rather than the old grouped visitor.

It is still only a contraction-core screen.  A win here would justify fusing the
surrounding triangle-multiplication LayerNorm/projection/gate/output boundary.
"""
...
def extension():
    ...
    _EXT = load(
        name="protenix_triangle_contraction_cutlass3_grouped_probe",
        sources=[str(REPO_ROOT / "scripts/perf/triangle_contraction_cutlass3_grouped_probe.cu")],
...
def main() -> None:
    ...
    result = {
        ...
        "shape": {
            ...
            "n_max": n_max,
            "n_cutlass": n_cutlass,
            "num_groups": args.features * len(lengths),
            "lengths": lengths,
            **efficiency(lengths),
        },
        "dense_torch_einsum": {
