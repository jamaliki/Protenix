...
def triton_fused_local_attention_bias_2d_enabled() -> bool:
    return os.getenv("PROTENIX_TRITON_LOCAL_ATTN_BIAS_2D", "0").lower() not in {
        "0",
        "false",
        "off",
        "no",
...
if triton_fused_local_attention_bias_available():
    ...
    @triton.jit
    def _project_local_attention_bias_2d_kernel(
        z_ptr,
        ln_weight_ptr,
        linear_weight_ptr,
        out_ptr,
        ...
        pid = tl.program_id(0)
        pair_offsets = (pid * block_pairs + tl.arange(0, block_pairs)).to(tl.int64)
        pair_mask = pair_offsets < total_pairs

        key = pair_offsets % n_keys
        tmp = pair_offsets // n_keys
        query = tmp % n_queries
        tmp = tmp // n_queries
        trunk = tmp % n_trunks
        sample = tmp // n_trunks
        ...
        z_base = (
            z_ptr
            + sample[:, None] * z_stride_s
            + trunk[:, None] * z_stride_t
            + query[:, None] * z_stride_q
            + key[:, None] * z_stride_k
        )
        channel = tl.arange(0, block_c)
        z = tl.load(
            z_base + channel[None, :] * z_stride_c,
            mask=pair_mask[:, None],
            other=0.0,
        ).to(tl.float32)

        sum_z = tl.sum(z, axis=1)
        sum_z2 = tl.sum(z * z, axis=1)
        mean = sum_z * (1.0 / block_c)
        var = sum_z2 * (1.0 / block_c) - mean * mean
        rstd = tl.rsqrt(var + eps)

        ln_w = tl.load(ln_weight_ptr + channel).to(tl.float32)
        w0 = ln_w * tl.load(linear_weight_ptr + channel).to(tl.float32)
        w1 = ln_w * tl.load(linear_weight_ptr + block_c + channel).to(tl.float32)
        w2 = ln_w * tl.load(linear_weight_ptr + 2 * block_c + channel).to(tl.float32)
        w3 = ln_w * tl.load(linear_weight_ptr + 3 * block_c + channel).to(tl.float32)
        weight_sum0 = tl.sum(w0, axis=0)
        weight_sum1 = tl.sum(w1, axis=0)
        weight_sum2 = tl.sum(w2, axis=0)
        weight_sum3 = tl.sum(w3, axis=0)

        weighted0 = tl.sum(z * w0[None, :], axis=1)
        weighted1 = tl.sum(z * w1[None, :], axis=1)
        weighted2 = tl.sum(z * w2[None, :], axis=1)
        weighted3 = tl.sum(z * w3[None, :], axis=1)
        ...
        tl.store(
            out_base,
            (weighted0 - mean * weight_sum0) * rstd,
            mask=pair_mask,
        )
        tl.store(
            out_base + out_stride_h,
            (weighted1 - mean * weight_sum1) * rstd,
            mask=pair_mask,
        )
        tl.store(
            out_base + 2 * out_stride_h,
            (weighted2 - mean * weight_sum2) * rstd,
            mask=pair_mask,
        )
        tl.store(
            out_base + 3 * out_stride_h,
            (weighted3 - mean * weight_sum3) * rstd,
            mask=pair_mask,
        )
...
def triton_project_local_attention_bias(
    ...
    *,
    ...
    if c_z == 16 and triton_fused_local_attention_bias_2d_enabled():
        block_pairs = _env_int(
            "PROTENIX_TRITON_LOCAL_ATTN_BIAS_2D_BLOCK_PAIRS", _BLOCK_PAIRS_2D
        )
        block_pairs = (
            block_pairs if block_pairs in {16, 32, 64, 128} else _BLOCK_PAIRS_2D
        )
        grid = (triton.cdiv(total_pairs, block_pairs),)
        _project_local_attention_bias_2d_kernel[grid](
            z_flat,
            layernorm_weight,
            linear_weight,
            out,
            ...
            block_c=c_z,
            num_warps=_env_int("PROTENIX_TRITON_LOCAL_ATTN_BIAS_2D_NUM_WARPS", 4),
        )
        if not batch_shape:
            return out[0]
