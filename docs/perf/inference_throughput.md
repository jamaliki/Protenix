...
A narrower version of the same idea was also rejected.  Commit `b3f0c45` added
`scripts/perf/triangle_attention_qkv_bias_probe.py`, which keeps the gate
projection and output epilogue unchanged and only folds the small triangle-bias
projection into the existing q/k/v GEMM.  This asks whether deleting the
separate bias GEMM and bias permute is enough without paying for the broader
q/k/v+gate+bias producer above.  H100 job `102407`
(`runs/triangle_qkv_bias_probe_20260704_075631_b3f0c45`) measured exact parity
but slower execution at every relevant base and v2 shape:

| shape | current ms | qkv+bias ms | speedup | parity |
| --- | ---: | ---: | ---: | --- |
| `B=16`, `N=124`, start | 0.8488 | 0.9770 | 0.8688x | exact |
| `B=16`, `N=124`, end | 0.8621 | 0.9856 | 0.8747x | exact |
| `B=16`, `N=220`, start | 3.1095 | 3.5373 | 0.8790x | exact |
| `B=16`, `N=220`, end | 3.1809 | 3.5894 | 0.8862x | exact |
| v2 `B=32`, `N=251`, start | 14.1109 | 15.4823 | 0.9114x | exact |
| v2 `B=32`, `N=251`, end | 14.4727 | 15.6860 | 0.9227x | exact |

Decision: keep the benchmark as negative evidence only.  The tiny bias
projection is not worth folding into q/k/v: adding even a few output columns to
the main GEMM plus a custom scatter is slower than the current separate
cuBLAS/Triton layout sequence.  This further raises the bar for the remaining
triangle-attention producer idea: it has to fuse LayerNorm and projection in a
vendor-class schedule, not just concatenate more projection weights.
...
1. **CuTe contraction screen:** replace only the `a,b -> update` triangular
   ...
   bucket, but the first implementation did not clear the long wide-v2 bucket
   that controls the Sam-style throughput target.  The long-bucket budget made
   the next bottleneck explicit: the current finish boundary alone cost about
   `2.75-2.79 ms`, leaving only about `1.35-1.49 ms` for the whole input
   producer if it was to beat the `4.1-4.2 ms` padded CUEQ update.

   Launch-level profile job `110660`, run
   `runs/triangle_update_direct_owned_profile_f9b7a65_retry_20260704_224443`,
   found the specific wrapper tax.  In the long shuffled outgoing direct-full
   profile, the producer was the expected `~1.82 ms` per update, but the finish
   boundary spent about `1.0 ms` per update in PyTorch `reshape`/`clone`/`copy_`
   while converting the exact-group `torch.bmm` output from `[record * D, N, N]`
   into row-major compact `[valid_rows, D]`.  The useful compute was much
   smaller: about `0.50 ms` in grouped `bmm`, `0.50 ms` in the output gated
   GEMM, `0.31 ms` in residual scatter, and `0.25 ms` in output LayerNorm.

   Tiled-assembly follow-up: commit `47f6861` replaced that
   `permute(...).reshape(...)` materialization with a small Triton tiled copy
   kernel in `assemble_update()`.  Job `110663`, run
   `runs/triangle_update_direct_owned_tiledassemble_47f6861_20260704_225010`,
   reran the same sorted and shuffled Sam-style v2 buckets:

   | bucket / direction | padded CUEQ update | direct producer only | tiled finish reusing producer | direct producer + tiled finish | decision |
   | --- | ---: | ---: | ---: | ---: | --- |
   | short shuffled incoming | `2.441 ms` | `0.437 ms` | `0.771 ms` (`3.17x`) | `1.202 ms` (`2.03x`) | advance to block gate |
   | short shuffled outgoing | `2.456 ms` | `0.439 ms` | `0.785 ms` (`3.13x`) | `1.216 ms` (`2.02x`) | advance to block gate |
   | long shuffled incoming | `4.126 ms` | `1.804 ms` | `1.780 ms` (`2.32x`) | `3.573 ms` (`1.15x`) | advance to block gate |
   | long shuffled outgoing | `4.207 ms` | `1.809 ms` | `1.816 ms` (`2.32x`) | `3.617 ms` (`1.16x`) | advance to block gate |

   Sorted and shuffled results again matched within noise; valid-region parity
   stayed at BF16 noise levels (`max_abs <= 0.03125`, no NaNs).  Decision:
   this is the first direct producer-owned wide-v2 triangle-multiplication
   update screen that is positive on both Sam-style buckets with producer cost
   included.  It is still not a model default because it is a benchmark boundary
   using PyTorch `bmm` groups and the current compact output gate, but it now
   deserves a full `PairformerBlock` integration gate.  The remaining kernel
   question is whether the grouped contraction/output boundary can be fused
   enough to survive block-level dilution across attention and transition.

   Full `PairformerBlock` gate: commit `bcc662b` added a `direct` producer mode
   to `scripts/perf/pairformer_compact_segmented_triangle_update_probe.py`, and
...
| Consuming exact-group contracted layout directly in the output finish | job `112768` skipped row-major update assembly and computed output stats/projection from `[record * channel, i, j]`; it lost to the current recompute-gate finish on both long updates (`3.086 -> 3.250 ms` incoming, `3.070 -> 3.292 ms` outgoing) and both short updates (`0.735 -> 0.793 ms`, `0.752 -> 0.829 ms`) with BF16-valid parity | the row-major update tensor is not just dead traffic; it gives output row stats and GEMMs coalesced channel access. A native kernel should emit an output-consumable layout or fuse into the contraction epilogue, not make the output finish read strided contracted global memory |
...
| Packed CUEQ triangle-attention q/k/v+bias producer | exact isolated screen, but slower on every base and v2 shape: `0.87-0.89x` for B16 base buckets and `0.91-0.92x` for v2 B32/N251 | even folding only the tiny triangle-bias projection into q/k/v loses; the remaining producer opportunity must be LayerNorm+projection fusion, not more concatenated projection weights |
...
- `scripts/perf/triangle_update_producer_owned_finish_probe.py`: upper-bound
  ...
  then times contraction plus the current output/residual boundary.  It now
  uses a Triton tiled assembly kernel instead of PyTorch
  ...
  enough for promotion.  The `contracted_recompute_gate` path is a rejected
  layout-boundary screen: directly reading the contracted
  `[record * channel, i, j]` layout in the output finish was slower than first
  assembling row-major update rows.
